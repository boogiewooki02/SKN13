...
select count(distinct dept_name) from emp; -- 부서가 몇 개
select count(distinct ifnull(dept_name, 'a')) from emp; -- 결측치를 포함한 개수
...
select if(salary < 10000, "10000 미만", "10000 이상"), 
	count(*) as "직원수"
from emp
...
-- 20명 이상인이 입사한 년도와 (그 해에) 입사한 직원수를 조회.
